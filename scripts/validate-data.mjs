...
const root=new URL("../data/",import.meta.url), grammar=JSON.parse(await readFile(new URL("grammar-topics.json",root))), topicIds=new Set(), setIds=new Set(), questionIds=new Set();
const errors=[];
...
for(const file of files){let set;try{set=JSON.parse(await readFile(new URL(`problem-sets/${file}`,root)))}catch(error){errors.push(`${file}: invalid JSON (${error.message})`);continue}
  ...
  for(const q of set.questions||[]){if(questionIds.has(q.id))errors.push(`${file}: duplicate question id ${q.id}`);questionIds.add(q.id);
    ...
    else{if(q.choices?.length!==4)errors.push(`${q.id}: expected 4 choices`);if(!q.choices?.some(c=>c.id===q.correctChoiceId))errors.push(`${q.id}: correct choice missing`)}
  }
}
